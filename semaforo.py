...
def estado_um():
    global estado, contador, diminuiTimer1, saiuModoNoturno
    estado = 2
    controlaContador.acquire()
    contador = 20
    diminuiTimer1 = 0
...
def FinalizaModoEspecial(signum, frame):
    global contador, estado, cruzamento, saiuModoNoturno
    if(estado >= 8):
        saiuModoNoturno = 1
    if(estado >= 7):
        contador = 0
...
def trataSinaisEmergência(signum, frame):
    global estado, contador
    estado = 7
    controlaContador.acquire()
    contador = 0
    controlaContador.release()
    print("Sinal de emergência ativado!")

def trataSinaisNoturno(signum, frame):
    global estado, contador
    estado = 9
    controlaContador.acquire()
    contador = 0
    controlaContador.release()
    print("Sinal noturno ativado!")
...
def conexaoServidor():
    while encerramento == 0:
        ...
        dest = (HOST, PORT)
        conectado = False
        dados = {"cruzamento":cruzamento, "pid":getpid()}
...
# main: 
signal.signal(signal.SIGUSR1, trataSinaisEmergência)
signal.signal(signal.SIGUSR2, trataSinaisNoturno)
signal.signal(signal.SIGTSTP, FinalizaModoEspecial)
